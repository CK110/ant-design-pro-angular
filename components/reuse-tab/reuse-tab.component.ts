...
@Component({
  ...
  template: `
    <nz-tabset #tabset
               ...
               [nzHideAdd]="true"
               [nzSelectedIndex]="pos"
               (nzClose)="myClose($event)">
      <ng-container *ngFor="let i of list; let index = index">
        <nz-tab [nzTitle]="titleTemplate" [nzClosable]="i.closable" (nzClick)="_to(index)"
                (nzContextmenu)="openMenu($event,i)">
