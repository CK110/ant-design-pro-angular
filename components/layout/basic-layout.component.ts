...
export class BasicLayoutComponent implements OnInit, OnChanges, OnDestroy {
  ...
  @Input() contentWidth: 'Fluid' | 'Fixed' = 'Fluid'; // layout 的内容模式,Fluid：定宽 1200px，Fixed：自适应
  ...
  ngOnInit() {
    ...
    this.router.events.pipe(
      ...
      this.openKeys = this.urlToList(this.router.url);
      this.cdf.markForCheck();
    });
  }
  ...
  private urlToList(url: string): string[] {
    if (url === '/') {
      return ['/'];
    }
